...
WORKINGDIR="/tmp/hivemind"
TMPFILE="hashes.tmp"

#this URL is used if a version number is not specified as an argument to the script
SIGNATUREFILE="https://hivemind.org/bin/0.9.2.1/SHA256SUMS.asc"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://hivemind.org/bin/"
VERSIONPREFIX="hivemind-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'hivemind-',
...
if [ $? -ne 0 ]; then
   ...
   echo "[hivemind-]<version>-[rc[0-9]] (example: hivemind-0.9.2-rc1)"
   echo "wget output:"
