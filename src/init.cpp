...
void Shutdown()
{
    ...
    RenameThread("hivemind-shutoff");
    mempool.AddTransactionsUpdated(1);
    StopRPCThreads();
#ifdef ENABLE_WALLET
    if (pwalletMain)
        bitdb.Flush(false);
    GenerateHiveminds(false, NULL, 0);
#endif
    StopNode();
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += "  -conf=<file>           " + strprintf(_("Specify configuration file (default: %s)"), "hivemind.conf") + "\n";
    if (mode == HMM_HIVEMINDD)
    {
#if !defined(WIN32)
...
#ifndef WIN32
    strUsage += "  -pid=<file>            " + strprintf(_("Specify pid file (default: %s)"), "hivemindd.pid") + "\n";
#endif
    ...
    strUsage += "  -port=<port>           " + strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 59533, 64533) + "\n";
    ...
    if (GetBoolArg("-help-debug", false))
    {
        strUsage += "  -checkpoints           " + strprintf(_("Only accept block chain matching built-in checkpoints (default: %u)"), 0) + "\n";
    ...
    strUsage += "  -rpcport=<port>        " + strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 59532, 64532) + "\n";
    ...
    strUsage += "\n" + _("RPC SSL options: (see the Hivemind Wiki for SSL setup instructions)") + "\n";
...
void ThreadImport(std::vector<boost::filesystem::path> vImportFiles)
{
    ...
    if (fReindex) {
        ...
        pmarkettree->WriteReindexing(false);
        fReindex = false;
...
/** Sanity checks
 *  Ensure that Hivemind is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Hivemind Core is shutting down."));
...
#endif
    // Make sure only a single Hivemind process is using the data directory.
    boost::filesystem::path pathLockFile = GetDataDir() / ".lock";
    FILE* file = fopen(pathLockFile.string().c_str(), "a"); // empty lock file; created if it doesn't exist.
    if (file) fclose(file);
    static boost::interprocess::file_lock lock(pathLockFile.string().c_str());
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Hivemind Core is probably already running."), strDataDir));
#ifndef WIN32
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Hivemind version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    size_t nMarketTreeDBCache = nTotalCache / 8;
    if (nMarketTreeDBCache > (1 << 21) && !GetBoolArg("-txindex", false))
        nMarketTreeDBCache = (1 << 21); // market tree db cache shouldn't be larger than 2 MiB
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                pmarkettree = new CMarketTreeDB(nMarketTreeDBCache, false, fReindex);
                ...
                if (fReindex) {
                    ...
                    pmarkettree->WriteReindexing(true);
				}
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Hivemind Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Hivemind Core to complete") << "\n";
    ...
    if (pwalletMain)
        GenerateHiveminds(GetBoolArg("-gen", false), pwalletMain, GetArg("-genproclimit", 1));
#endif
