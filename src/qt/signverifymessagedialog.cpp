    ...
    model(0)
{
    ...
    ui->signatureOut_SM->setFont(GUIUtil::hivemindAddressFont());
    ui->signatureIn_VM->setFont(GUIUtil::hivemindAddressFont());
}
...
void SignVerifyMessageDialog::on_signMessageButton_SM_clicked()
{
    ...
    CHivemindAddress addr(ui->addressIn_SM->text().toStdString());
    if (!addr.IsValid())
    {
...
{
    CHivemindAddress addr(ui->addressIn_VM->text().toStdString());
    if (!addr.IsValid())
    {
    ...
    if (!(CHivemindAddress(pubkey.GetID()) == addr))
    {
        ui->statusLabel_VM->setStyleSheet("QLabel { color: red; }");
