...
const QString HivemindGUI::DEFAULT_WALLET = "~Default";

HivemindGUI::HivemindGUI(const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    clientModel(0),
    walletFrame(0),
    ...
    spinnerFrame(0)
{
    ...
    QString windowTitle = tr("Hivemind Core") + " - ";
#ifdef ENABLE_WALLET
...
void HivemindGUI::createMenuBar()
{
#ifdef Q_OS_MAC
...
void HivemindGUI::setClientModel(ClientModel *clientModel)
{
...
#ifdef ENABLE_WALLET
bool HivemindGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool HivemindGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void HivemindGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void HivemindGUI::setWalletActionsEnabled(bool enabled)
{
...
void HivemindGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
    trayIcon = new QSystemTrayIcon(this);
    QString toolTip = tr("Hivemind Core client") + " " + networkStyle->getTitleAddText();
...
void HivemindGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void HivemindGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void HivemindGUI::optionsClicked()
{
...
void HivemindGUI::showHelpMessageClicked()
{
...
void HivemindGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void HivemindGUI::gotoHistoryPage()
{
...
void HivemindGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void HivemindGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void HivemindGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}

void HivemindGUI::gotoResolveVoteTab()
{
...
void HivemindGUI::setNumConnections(int count)
{
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to Hivemind network", "", count));
}

void HivemindGUI::setNumBlocks(int count)
{
    if(!clientModel)
        return;
...
void HivemindGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Hivemind"); // default title
...
void HivemindGUI::changeEvent(QEvent *e)
{
...
void HivemindGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
#ifdef ENABLE_WALLET
void HivemindGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address)
{
    ...
    message((amount)<0 ? tr("Sent transaction") : tr("Incoming transaction"),
             tr("Date: %1\n"
                ...
                "Address: %4\n")
                  .arg(date)
                  .arg(HivemindUnits::formatWithUnit(unit, amount, true))
                  .arg(type)
                  .arg(address), CClientUIInterface::MSG_INFORMATION);
}
#endif // ENABLE_WALLET

void HivemindGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void HivemindGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool HivemindGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool HivemindGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void HivemindGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void HivemindGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void HivemindGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void HivemindGUI::detectShutdown()
{
...
void HivemindGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
static bool ThreadSafeMessageBox(HivemindGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void HivemindGUI::subscribeToCoreSignals()
{
    // Connect signals to client
    uiInterface.ThreadSafeMessageBox.connect(boost::bind(ThreadSafeMessageBox, this, _1, _2, _3));
}

void HivemindGUI::unsubscribeFromCoreSignals()
{
    ...
    menu(0)
{
    ...
    QList<HivemindUnits::Unit> units = HivemindUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    foreach (const HivemindUnits::Unit unit, units)
    {
        max_width = qMax(max_width, fm.width(HivemindUnits::name(unit)));
    }
...
{
    menu = new QMenu();
    foreach(HivemindUnits::Unit u, HivemindUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(HivemindUnits::name(u)), this);
...
{
    setText(HivemindUnits::name(newUnits));
}
